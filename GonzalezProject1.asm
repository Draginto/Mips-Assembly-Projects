...
main:
	...
	div $s2, $s3 # sum/n = ...
	mfhi $t0 # The remainder.
	
	addi $t2, $zero, 10 # Get the multiplier(10).
	
	mult $t0, $t2 # multiply the current remainder in $t0 by 10.
	mflo $s0 # Keep the new remainder in $s0.
		
	div $s0, $s3 # Divide the new remainder by n.
	mflo $t1 # Stores new quotient to $t1. 
	mfhi $t5 # Stores new remainder to $t5. 
	
	mult $t5, $t2 # mult by 10, second integer.
	mflo $s1 # store value.
	...
	mult $t4, $t2 # mult by 10, second integer.
	mflo $s2 # store value.
	
	div $s2, $s3
	mflo $t4 # Quotient
	
	move $a1, $t1 # Move first decimal to $a1.
	move $s5, $t3  # Move second decimal to $s5.
	move $s6, $t4 # Add last decimal place to $s6.
	...
	move $a0, $s5
	li $v0, 1
	syscall # Prints 2nd decimal.
	
	move $a0, $s6
	li $v0, 1
	syscall # Prints 3rd decimal.
